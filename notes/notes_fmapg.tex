...
\begin{align}
  ...
  \Rightarrow \qquad \qquad \qquad \qquad A_{(s, :), (s, :)} &= d^{\pi_t}(s) \Big( \text{diag} (p^{\pi_\theta}(\cdot | s)) - p^{\pi_\theta}(\cdot | s) p^{\pi_\theta}(\cdot | s)^\top \Big),
\end{align}
...
\begin{align}
  & \frac{\partial}{\partial p^\pi(b | x)} \frac{\partial}{\partial \theta(s', a')} \mathcal{C}_{\text{MDPO}} \nonumber \\
  &= \frac{\partial}{\partial p^\pi(b | x)} d^{\pi_t}(s') p^\pi(a' | s') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - \sum_c p^\pi(c | s') \log \frac{p^\pi(c | s')}{p^{\pi_t}(c | s')} \right) \nonumber \\
  &= \mathbb{I}(x = s') d^{\pi_t}(s') \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - \sum_c p^\pi(c | s') \frac{p^\pi(c | s')}{p^{\pi_t}(c | s')} \right) + p^\pi(a' | s') \frac{\mathbb{I}(b = a')}{p^\pi(a' | s')} \nonumber \\
    & \qquad \qquad \qquad \qquad \quad - p^\pi(a' | s') \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \Bigg] \nonumber \\
  &= \mathbb{I}(x = s') d^{\pi_t}(s') \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) - p^\pi(a' | s') \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \Bigg],
\end{align}
where we introduced the intermediate variable $T_{\text{KL}}(s') :=\text{KL}(p^\pi(\cdot | s') \| p^{\pi_t}(\cdot | s') = \sum_c p^\pi(c | s') \frac{p^\pi(c | s')}{p^{\pi_t}(c | s')}$.

\begin{align}
  \frac{\partial}{\partial \theta(s, a)} \frac{\partial \mathcal{C}_{\text{MDPO}}}{\partial \theta(s', a')} &= \sum_{x, b} \frac{\partial p^\pi(b | x)}{\partial \theta(s, a)} \frac{\partial}{\partial p^\pi(b | x)} \frac{\partial \mathcal{C}_{\text{MDPO}}}{\partial \theta(s', a')} \nonumber \\
  &= {\color{red!67!black}{\sum_{{\color{black}{x}}, b}}} \mathbb{I}(x = s) {\color{red!67!black}{\Big[ \mathbb{I}(b = a) - p^\pi(a | x) \Big] p^\pi(b | x)}} \times \mathbb{I}(x = s') d^{\pi_t}(s') \nonumber \\
  & \qquad \quad {\color{red!67!black}{\times \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) - p^\pi(a' | s') \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \Bigg]}} \nonumber \\
  &= \mathbb{I}(s = s') d^{\pi_t}(s') \times {\color{red!67!black}{T_{\text{aux}}}},
\end{align}
where the auxillary term $T_{\text{aux}}$ is
\begin{align}
  T_{\text{aux}} &= \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) \sum_{b} \mathbb{I}(b = a) p^\pi(b | s) \mathbb{I}(b = a') \nonumber \\
  & \qquad - p^\pi(a' | s') \sum_{b} \mathbb{I}(b = a) p^\pi(b | s) \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \nonumber \\
  & \qquad - p^\pi(a | s) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) \sum_{b} p^\pi(b | s) \mathbb{I}(b = a') \nonumber \\
  & \qquad + p^\pi(a' | s') p^\pi(a | s) \sum_{b} p^\pi(b | s) \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \nonumber \\  
  &= \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) p^\pi(a | s) \mathbb{I}(a = a') - p^\pi(a' | s') p^\pi(a | s) \left( \log \frac{p^\pi(a | s')}{p^{\pi_t}(a | s')} + 1 \right) \nonumber \\
  & \qquad - p^\pi(a | s) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) p^\pi(a' | s) + p^\pi(a' | s') p^\pi(a | s) \left( T_{\text{KL}}(s') + 1 \right) \nonumber \\
  &= \mathbb{I}(a = a') p^\pi(a | s) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) \nonumber \\
  & \qquad - 2 p^\pi(a' | s') p^\pi(a | s) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) + p^\pi(a' | s') p^\pi(a | s).
\end{align}
Therefore, 
\begin{align}
  & \frac{\partial}{\partial \theta(s, a)} \frac{\partial}{\partial \theta(s', a')} \mathcal{C}_{\text{MDPO}} \nonumber \\
  &= \mathbb{I}(s = s') d^{\pi_t}(s') \left[ p^\pi(a | s) \Big(\mathbb{I}(a = a') - 2 \Big) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) + p^\pi(a' | s') p^\pi(a | s) \right],
\end{align}
and
\begin{align}
  A_{(s, :), (s, :)} &= d^{\pi_t}(s) \cdot \Big(\mathbf{I} - \mathbf{1}_{|\mathcal{A}| \times |\mathcal{A}|} \Big) \odot p^\pi(\cdot | s) \Big[ \log \Big( p^\pi(\cdot | s) \oslash p^{\pi_t}(\cdot | s) \Big) - \Big(T_{\text{KL}}(s) - 1 \Big) \mathbf{1}_{|\mathcal{A}|} \Big]^\top \nonumber \\
  & \qquad + d^{\pi_t}(s) \cdot p^{\pi}(\cdot | s) p^{\pi}(\cdot | s)^\top.
\end{align}
