...
\begin{equation}
  \max_\theta \; \underbrace{\left[ \sum_s d^{\pi_t}(s) \sum_a p^{\pi_\theta}(a | s) Q^{\pi_t}(s, a) - \delta \sum_s d^{\pi_t}(s) \cdot \text{KL}(p^{\pi_t}(\cdot | s) \| p^{\pi_\theta}(\cdot | s)) \right]}_{=: \mathcal{J}_{\text{TRPO-KL}} = \mathcal{J}_{\text{TRPO}} - \delta \mathcal{C}_{\text{TRPO}}}.  
\end{equation}
...
\begin{align}
  \frac{\partial \mathcal{J}_{\text{TRPO-KL}}}{\partial \theta(s, a)} &= d^{\pi_t}(s) p^{\pi_\theta}(a | s) \left[ Q^{\pi_t}(s, a) - \sum_b p^{\pi_\theta}(b | s) Q^{\pi_t}(s, b) \right] - \delta d^{\pi_t}(s) \Big[ p^{\pi_\theta}(a | s) - p^{\pi_t}(a | s) \Big].
\end{align}
...
\begin{table}[!hbt]
  ...
  \begin{tabular}{c|l|c}
    ...
    \hline
    TRPO &
    $\sum_s d^{\pi_t}(s) \sum_a p^{\pi_\theta}(a | s) Q^{\pi_t}(s, a)$ &
    \texttt{(same as above)} \\
    \hline
    MDPO &
    $\sum_s d^{\pi_t}(s) \sum_a p^{\pi_\theta}(a | s) A^{\pi_t}(s, a)$ &
...
\begin{table}[!hbt]
  ...
  \caption{The gradients of the objectives and constraints wrt. the policy parameter corresponding to the different PG algorithms. Note that the gradient of the objective for both TRPO and MDPO is exactly equal to each other.}
