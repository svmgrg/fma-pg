...
\begin{align}
  ...
  &= d^{\pi_t}(s) \Big[ p^{\pi_t}(a | s) - p^{\pi_\theta}(a | s) \Big].
\end{align}
Finally, using the above result yields
\begin{IEEEeqnarray}{lrCl}
  & \frac{\partial}{\partial \theta(s, a)} \frac{\partial}{\partial \theta(s', a')} \mathcal{C} &=& \frac{\partial}{\partial \theta(s, a)} d^{\pi_t}(s') \Big[ p^{\pi_t}(a' | s') - p^{\pi_\theta}(a' | s') \Big] \nonumber \\
