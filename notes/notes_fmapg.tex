...
\begin{align}
  ...
  &= \mathbb{I}(x = s') d^{\pi_t}(s') \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - \sum_c p^\pi(c | s') \frac{p^\pi(c | s')}{p^{\pi_t}(c | s')} \right) \nonumber \\
...
\end{align}
where we introduced the intermediate variable $T_{\text{KL}}(s') :=\text{KL}(p^\pi(\cdot | s') \| p^{\pi_t}(\cdot | s') = \sum_c p^\pi(c | s') \frac{p^\pi(c | s')}{p^{\pi_t}(c | s')}$. Then using the law of total derivative, we obtain
\begin{align}
  ...
  & \qquad \quad \times \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) - p^\pi(a' | s') \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \Bigg] \nonumber \\
  &= \mathbb{I}(s = s') d^{\pi_t}(s') \times T_{\text{aux}},
\end{align}
where the auxillary term $T_{\text{aux}}$ is defined as
\begin{align}
  T_{\text{aux}} &= \sum_b \Big[ \mathbb{I}(b = a) - p^\pi(a | s') \Big] p^\pi(b | s') \times \Bigg[ \mathbb{I}(b = a') \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) \nonumber \\
    & \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad \qquad - p^\pi(a' | s') \left( \log \frac{p^\pi(b | s')}{p^{\pi_t}(b | s')} + 1 \right) \Bigg]
\end{align}
Let us now simplify $T_{\text{aux}}$:
\begin{align}
...
Therefore, 
\begin{align}
  \frac{\partial}{\partial \theta(s, a)} \frac{\partial \mathcal{C}_{\text{MDPO}}}{\partial \theta(s', a')} &= \mathbb{I}(s = s') d^{\pi_t}(s') \bigg[ p^\pi(a | s') \Big(\mathbb{I}(a = a') - 2 p^\pi(a' | s') \Big) \left( \log \frac{p^\pi(a' | s')}{p^{\pi_t}(a' | s')} - T_{\text{KL}}(s') + 1 \right) \nonumber \\
    & \qquad \qquad \qquad \qquad \quad + p^\pi(a' | s') p^\pi(a | s') \bigg],
\end{align}
and
\begin{align}
  A_{(s, :), (s, :)} &= d^{\pi_t}(s) \cdot \Big[ \text{diag} (p^{\pi}(\cdot | s)) - 2 p^{\pi}(\cdot | s) p^{\pi}(\cdot | s)^\top \Big] \Big[ \log \Big( p^\pi(\cdot | s) \oslash p^{\pi_t}(\cdot | s) \Big) - \Big(T_{\text{KL}}(s) - 1 \Big) \mathbf{1}_{|\mathcal{A}|} \Big]^\top \nonumber \\
